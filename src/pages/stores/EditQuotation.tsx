...
const EditQuotationPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!quotationId) {
      message.error('معرف عرض السعر غير صحيح');
      navigate('/stores/quotations');
    }
  }, [quotationId, navigate]);
  
  // حالة التحميل لبيانات عرض السعر
  const [loadingQuotation, setLoadingQuotation] = useState<boolean>(true);
  const [quotationExists, setQuotationExists] = useState<boolean>(false);
...
interface CompanyData {
  ...
  // دالة جلب بيانات عرض السعر المحدد للتعديل
  const fetchQuotationData = useCallback(async () => {
    if (!quotationId) {
      customMessage.error('معرف عرض السعر غير صحيح');
      navigate('/stores/quotations');
      return;
    }
    ...
    try {
      ...
      if (!quotationSnap.exists()) {
        ...
        customMessage.error('عرض السعر غير موجود');
        navigate('/stores/quotations');
        return;
      }
      ...
      setQuotationData({
        ...
        cashBox: quotationData.cashBox || '',
        multiplePayment: quotationData.multiplePayment || {},
        ...
        priceRule: quotationData.priceRule || '',
        commercialRecord: quotationData.commercialRecord || '',
        taxFile: quotationData.taxFile || '',
      ...
      if (quotationData.items && Array.isArray(quotationData.items)) {
        const formattedItems = quotationData.items.map((item: FirebaseQuotationItem) => ({
          ...
          taxPercent: String(item.taxPercent || taxRate),
      ...
      // تحديث الإجماليات
      if (quotationData.totals) {
        setTotals({
          afterDiscount: Number(quotationData.totals.afterDiscount || 0),
          afterTax: Number(quotationData.totals.afterTax || 0),
          total: Number(quotationData.totals.total || 0),
          tax: Number(quotationData.totals.tax || 0)
        });
      }
      ...
      customMessage.success('تم تحميل بيانات عرض السعر بنجاح');
    } catch (error) {
      console.error('خطأ في جلب بيانات عرض السعر:', error);
      customMessage.error('حدث خطأ في تحميل بيانات عرض السعر');
      setQuotationExists(false);
    } finally {
      setLoadingQuotation(false);
    }
  }, [quotationId, navigate, customMessage, taxRate]);

  // جلب بيانات عرض السعر عند تحميل الصفحة
  useEffect(() => {
    const loadData = async () => {
      // أولاً جلب البيانات الأساسية
      await fetchBasicData();
      // ثم جلب بيانات عرض السعر
      await fetchQuotationData();
    };
    
    loadData();
  }, [fetchBasicData, fetchQuotationData]);
...
interface SavedQuotation {
  ...
  const handleSave = async () => {
    if (items.length === 0) {
      customMessage.error('لا يمكن تحديث عرض السعر بدون أصناف');
      return;
    }
    ...
    try {
      // تحديث عرض السعر في Firestore
      const quotationRef = doc(db, 'quotations', quotationId);
      await updateDoc(quotationRef, quotation);
      customMessage.success('تم تحديث عرض السعر بنجاح!');
      ...
      message.error('حدث خطأ أثناء تحديث عرض السعر');
    } finally {
      setLoading(false);
    }
  };
...
const handlePrint = () => {
  ...
  return (
    <div className="p-2 sm:p-6 w-full max-w-none">
      {/* حالة التحميل أثناء جلب بيانات عرض السعر */}
      {loadingQuotation && (
        <div className="flex justify-center items-center min-h-screen">
          <Spin size="large" tip="جاري تحميل بيانات عرض السعر..." />
        </div>
      )}

      {/* حالة عدم وجود عرض السعر */}
      {!loadingQuotation && !quotationExists && (
        <div className="flex flex-col justify-center items-center min-h-screen">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-600 mb-4">عرض السعر غير موجود</h2>
            <p className="text-gray-500 mb-6">لم يتم العثور على عرض السعر المطلوب</p>
            <Button type="primary" onClick={() => navigate('/stores/quotations')}>
              العودة إلى قائمة عروض الأسعار
            </Button>
          </div>
        </div>
      )}
      ...
      <div className="p-4 font-['Tajawal'] bg-white mb-4 rounded-lg shadow-[0_0_10px_rgba(0,0,0,0.1)] relative overflow-hidden">
        ...
        <p className="text-gray-600 mt-2">
          تعديل عرض السعر{quotationData.quotationNumber ? ` رقم: ${quotationData.quotationNumber}` : ''}
        </p>
        ...
        إضافة
          ...
          <Row justify="center" gutter={12}>
            <Col>
              <Button
                size="large"
                onClick={() => navigate('/stores/quotations')}
                style={{ width: 120, backgroundColor: '#6b7280', borderColor: '#6b7280', color: '#fff' }}
              >
                إلغاء
              </Button>
            </Col>

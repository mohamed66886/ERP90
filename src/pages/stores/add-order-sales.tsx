...
interface SalesOrderData {
  ...
  taxFile: string;
  dueDate?: string;
  validUntil?: string; // تاريخ التوريد
}
...
const initialItem: SalesOrderItem = {
  itemNumber: '',
  itemName: '',
  quantity: '',
  unit: 'قطعة',
...
// دالة توليد رقم أمر بيع جديد بناءً على رقم الفرع والتاريخ والتسلسل اليومي
async function generateSalesOrderNumberAsync(branchId: string, branches: Branch[] = []): Promise<string> {
  const date = new Date();
  const y = date.getFullYear();
  ...
  try {
    // جلب عدد أوامر البيع لنفس الفرع في نفس السنة
    const { getDocs, collection, query, where } = await import('firebase/firestore');
    const q = query(
    ...
    // فلترة النتائج في الكود بدلاً من قاعدة البيانات لتجنب مشكلة الفهرس
    const currentYearSalesOrders = snapshot.docs.filter(doc => {
      const salesOrderDate = doc.data().date;
      if (!salesOrderDate) return false;
      const salesOrderYear = new Date(salesOrderDate).getFullYear();
      return salesOrderYear === y;
    });
    
    const count = currentYearSalesOrders.length + 1;
    const serial = count;
    
    return `SO-${branchNumber}-${y}-${serial}`;
  } catch (error) {
    console.error('خطأ في توليد رقم أمر البيع:', error);
    // في حالة الخطأ، استخدم رقم عشوائي
    const randomSerial = Math.floor(Math.random() * 1000) + 1;
    return `SO-${branchNumber}-${y}-${randomSerial}`;
  }
}
...
// دالة حساب تاريخ انتهاء صلاحية أمر البيع (بعد 30 يوم من تاريخ أمر البيع)
function calculateValidUntilDate(salesOrderDate: string): string {
  if (!salesOrderDate) return '';
  return dayjs(salesOrderDate).add(30, 'day').format('YYYY-MM-DD');
}

const AddSalesOrderPage: React.FC = () => {
  // حالة الرسائل
...
interface CompanyData {
  ...
  const [quotationData, setQuotationData] = useState<SalesOrderData>({
    quotationNumber: '',
  ...
  const [item, setItem] = useState<SalesOrderItem & { warehouseId?: string }>(initialItem);
...
interface FirebaseSalesOrderItem {
  itemNumber?: string;
  itemName?: string;
...
interface SavedQuotation {
  ...
  const [lastSavedSalesOrder, setLastSavedSalesOrder] = useState<SavedQuotation | null>(null);
    ...
    try {
      ...
      invoicesSnap.forEach(doc => {
        ...
        if (Array.isArray(data.items)) {
          data.items.forEach((item: FirebaseSalesOrderItem) => {
  ...
  const fetchLastCustomerPrice = async (customerName: string, itemName: string) => {
    try {
      ...
      const filtered = salesSnap.docs
        ...
        .flatMap(inv => inv.items.filter((it: FirebaseSalesOrderItem) => it.itemName === itemName))
        .sort((a: FirebaseSalesOrderItem, b: FirebaseSalesOrderItem) => new Date(String(b.date || '')).getTime() - new Date(String(a.date || '')).getTime());
  ...
  const calculateItemValues = (item: SalesOrderItem) => {
  ...
  const addItem = async () => {
    ...
    const newItem: SalesOrderItem & { warehouseId?: string; mainCategory?: string; cost?: number } = {
      ...item,
      itemNumber: item.itemNumber || 'N/A',
  ...
  const handleSave = async () => {
    ...
    if (quotationData.date && !validateDate(quotationData.date)) {
      customMessage.error(`تاريخ أمر البيع خارج نطاق السنة المالية. ${getDateValidationMessage(quotationData.date)}`);
      return;
    }
    ...
    setLoading(true);
    // حذف الحقول الفارغة من بيانات أمر البيع
    ...
    try {
      ...
      setLastSavedSalesOrder(quotation as unknown as SavedQuotation);
      ...
      if (warehouseMode === 'single' && quotationData.warehouse) {
        ...
        for (const savedItem of items) {
          const itemWithWarehouse = savedItem as SalesOrderItem & { warehouseId?: string };
  ...
  const itemColumns = [
    ...
    // إظهار عمود المخزن فقط إذا كان وضع المخزن multiple
    {
      ...
      width: 120,
      align: 'center' as const,
      render: (_: string | undefined, record: SalesOrderItem & { warehouseId?: string }) => {
    ...
    },
    {
      ...
      width: 140,
      align: 'center' as const,
      render: (_: unknown, record: SalesOrderItem, index: number) => (
  ...
  const handleEditItem = (record: SalesOrderItem, index: number) => {
    setItem({
      ...record,
      taxPercent: taxRate // استخدام نسبة الضريبة من إعدادات الشركة
    });
  ...
  const handleEditInvoice = (record: InvoiceRecord & { firstLevelCategory?: string }) => {
    ...
    setLastSavedSalesOrder(record as unknown as SavedQuotation);
...
const handlePrint = () => {
    ...
    (async () => {
      ...
      const invoice = lastSavedSalesOrder;
      if (!invoice) {
      ...
      printContainer.innerHTML = `
        ...
        <body>
          ...
          <table>
            ...
            <tbody>
              ${(invoice.items || []).map((it: SalesOrderItem & { warehouseId?: string }, idx: number) => {
              ...
              <tr style="background:#f3f3f3; font-weight:bold;">
                ...
                <td style="color:#dc2626; font-weight:bold;">
                  ${(() => {
                    ...
                    let total = 0;
                    invoice.items.forEach((it: SalesOrderItem) => { total += Number(it.discountValue) || 0; });
                ...
                <td style="color:#ea580c; font-weight:bold;">
                  ${(() => {
                    ...
                    if (!invoice.items) return '0.00';
                    let total = 0;
                    invoice.items.forEach((it: SalesOrderItem) => {
                      const subtotal = Number(it.price) * Number(it.quantity);
                      const discountValue = Number(it.discountValue) || 0;
                ...
                <td style="color:#9333ea; font-weight:bold;">
                  ${(() => {
                    ...
                    if (!invoice.items) return '0.00';
                    let total = 0;
                    invoice.items.forEach((it: SalesOrderItem) => { total += Number(it.taxValue) || 0; });
                    return total.toFixed(2);
                  })()}
                </td>
    ...
    <div className="p-2 sm:p-6 w-full max-w-none">
      ...
      <div className="p-4 font-['Tajawal'] bg-white mb-4 rounded-lg shadow-[0_0_10px_rgba(0,0,0,0.1)] relative overflow-hidden">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800">أمر بيع جديد</h1>
        </div>
        <p className="text-gray-600 mt-2">إدارة أوامر البيع</p>
              ...
              <Form.Item label="التاريخ">
                <DatePicker
                  ...
                  onChange={(date, dateString) => {
                    ...
                    if (newDate && !handleDateValidation(newDate, 'تاريخ أمر البيع')) {
              ...
              <Form.Item label="الفرع">
                <Select
                  ...
                  onChange={async (value) => {
                    try {
                      setBranchCode('');
                      const quotationNumber = await generateSalesOrderNumberAsync(value, branches);
          ...
          <Row justify="center" gutter={12}>
                ...
                onClick={async () => {
                  if (Number(totalsDisplay.net) <= 0) {
                    if (typeof message !== 'undefined' && message.error) {
                      message.error('لا يمكن حفظ أمر البيع إذا كان الإجمالي النهائي صفر أو أقل');
                    } else {
                      alert('لا يمكن حفظ أمر البيع إذا كان الإجمالي النهائي صفر أو أقل');
                    }
                    return;
                  }
                  ...
                  // بعد الحفظ: توليد رقم أمر بيع جديد لأمر البيع التالي
                  const newInvoiceNumber = await generateSalesOrderNumberAsync(quotationData.branch, branches);
                  setQuotationData(prev => ({
                    ...prev,
                    quotationNumber: newInvoiceNumber
                  }));
                }}
            ...
            <Col>
              <Button
                ...
                onClick={handlePrint}
                disabled={loading || !lastSavedSalesOrder}
                style={{ width: 150, backgroundColor: '#60a5fa', borderColor: '#60a5fa', color: '#fff' }}
              >
                طباعة الفاتورة
              </Button>
